...
pvedaemon or the write path. **Writing a guest's `publish` key is root inside that
...
* **The prefix.** `/usr/share/pve-meta/prefixes/publish.yaml`: `selector: { all: true
  }`, `enforce: true`; a cluster or node file of the same name overrides it (§3). A
  write is refused only when `publish` is not a map; the daemon checks each entry's
  fields.
...
* **Entries are judged one by one.** An entry that fails to validate — an unknown or
  ...
  it published before. Two entries resolving to one path, or one to a directory the
  other's path runs through, are both refused. A document that does not parse, or a
  `publish` that is not a map, holds everything.
* **Paths.** A relative path resolves under `/etc/pve-meta`, an absolute one is used as
  ...
  carrying `.pve-meta-publish` (the temp mark); nothing is normalised, and a path has no
...
* **The daemon**, `pve-meta-publish daemon`, runs as root on every node, on the
  ...
  document has `publish`, or its manifest still has records. A watched, running
  ...
  (`lock:`) until it clears. One `flock` per guest under `/run/lock/pve-meta-publish` is
