...
- [x] Run `lintian` from the `deb`/`dsc` targets (both `Makefile:deb` and `pve-ext/Makefile:deb`; fatal when `$CI` is set, `|| true` locally — see `pve-ext/README.md`, "Managed patches" build notes, and `docs/DESIGN.md` §14).
