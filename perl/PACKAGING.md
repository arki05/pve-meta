...
## The namespace and grant drop-directories

`docs/DESIGN.md` §3 puts namespaces (what a prefix is) and grants (who may touch one)
outside the documents, in three directories:

* `/usr/share/pve-meta/namespaces/` — packaged namespaces, one file per prefix; **the
  file name is the prefix** (`docs/DESIGN.md` §3.1). The root `Makefile`'s `install:`
  target already creates this directory and copies `namespaces/*.yaml` into it, tolerating
  an empty checkout (a missing directory is not an error). This repository's
  `namespaces/traefik.yaml` documents the format rather than granting anything — a
  namespace names no principal at all — so packaging that one file is optional, but the
  directory itself is the ecosystem seam: an operator's own `.deb` drops its namespace
  file in here.
* `/etc/pve/meta.d/namespaces/` — cluster-wide overrides on pmxcfs. `debian/pve-meta.postinst`
  creates it (guarded on `/etc/pve/local`, i.e. only on a node that has joined a cluster);
  nothing packages files into it.
* `/etc/pve/meta.d/grants/` — grants, created by the same postinst. **Cluster-only: there
  is deliberately no packaged grants directory** (`docs/DESIGN.md` §3.2). An operator's
  `.deb` may ship a namespace, which is a declaration, but must never ship its own grant,
...
A cluster namespace file overrides the packaged file of the same name; all of these are
...
single registration listing revision 6 has since split into `namespaces` and `grants`)
