...
async function loadStablePage(browser, t, result, maxAttempts) {
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        await page.setCookie({ name: 'PVEAuthCookie', value: t.ticket, domain: host, path: '/', secure: true }, { name: 'PVEThemeCookie', value: 'proxmox-dark', domain: host, path: '/', secure: true });
        await page.evaluateOnNewDocument((csrf) => {
            // evaluateOnNewDocument runs in EVERY new document context on
            // this page, including our own Metadata iframe (a different
            // origin: host:8007). sessionStorage access can throw a
            // SecurityError in some such contexts (e.g. the browser's
            // built-in network-error page shown if that origin is
            // unreachable) - guard it so that never surfaces as a
            // spurious page error unrelated to the patch under test.
            try {
        ...
        if (initialOk && earlyErrors.length === 0) {
            ...
            page.on('requestfailed', (req) => {
                if (/pve-meta-loader\.js|:8007\//.test(req.url())) {
...
async function main() {
    ...
    try {
        ...
        result.checks.pveMetaLoaded = await page.evaluate(() => window.PveMetaLoaded === true);
        ...
        const iframeInfo = await page.evaluate(() => {
            const f = document.querySelector('iframe[src*=":8007/ui/"]');
            if (!f) return null;
            const rect = f.getBoundingClientRect();
            return { src: f.src, w: rect.width, h: rect.height };
        });
        result.checks.iframe = iframeInfo;

        if (!iframeInfo) throw new Error('no matching iframe (src containing :8007/ui/) found after clicking Metadata');

        const expectedQuery = kind === 'dc' ? 'dc=1' : `vmid=${vmid}`;
        if (!iframeInfo.src.includes(expectedQuery)) {
            throw new Error(`iframe src missing expected query "${expectedQuery}": ${iframeInfo.src}`);
        }
        if (!/theme=(light|dark)/.test(iframeInfo.src)) {
            throw new Error('iframe src missing theme=light|dark: ' + iframeInfo.src);
        }
        if (!(iframeInfo.w > 50 && iframeInfo.h > 50)) {
            throw new Error(`iframe rendered too small to be "filling the tab": ${iframeInfo.w}x${iframeInfo.h}`);
        }
