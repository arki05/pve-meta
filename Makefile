...
UI_DIR := ui-extjs
# The editor is written as ui-extjs/src/*.js, one file per section, and shipped
# as the one script pve-ext's page loader fetches: the sources concatenated in
# name order (the two-digit prefix is the order), behind one line saying so.
# No bundler, no transform; `node --check` and the smoke suite run on the
# result, which is exactly what the package installs.
...
# docs/reference/PROXMOX-CONVENTIONS.md section 7.6/8, docs/DESIGN.md section 14,
